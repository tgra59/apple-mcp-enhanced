...
async function checkMapsAccess(): Promise<boolean> {
  try {
    ...
    console.error(
      `Cannot access Maps app: ${error instanceof Error ? error.message : String(error)}`,
    );
    return false;
  }
}
...
async function saveLocation(
  ...
  try {
    ...
    console.error(
      `saveLocation - Saving location: "${name}" at address "${address}"`,
    );

    const result = (await run(
      (args: { name: string; address: string }) => {
        try {
          const Maps = Application("Maps");
          Maps.activate();
...
async function getDirections(
  ...
  try {
    ...
    console.error(
      `getDirections - Getting directions from "${fromAddress}" to "${toAddress}"`,
    );
...
async function dropPin(name: string, address: string): Promise<SaveResult> {
  try {
    ...
    console.error(
      `dropPin - Creating pin at: "${address}" with name "${name}"`,
    );
